...
#cores
cores = {'vermelho' : '\033[31m', 'Azul' : '\033[34m', 'Verde' : '\033[32m', 'roxo'  : '\033[35m', 'vazio' : '\033[m'}
...
def nova_partida(Pontos_pc, Pontos_player): #=={Define a condição de nova partida}
    nova = int( input( 'Deseja começar uma nova partida?\n Sim [1] Não [2]\nDigite aqui:' ) )
...
def voce_ganhou(player, pc): #=={Alerta de vítoria}
    ...
    print(f'Você ganhou esse round!!\n Sua escolha foi {player} e a do seu oponente foi {pc}')
    print(f'--Seus Pontos {Pontos_player} x Pontos do Pc {Pontos_pc}--')
    print(f'{cores["Verde"]}{'-+-' * 10}{cores["vazio"]}')

def vitorias(player, pc):#=={Define a condição de vítoria}
    return(player == 0 and (pc == 1 or pc == 2) or
       player == 1 and (pc == 2 or pc == 3) or
       player == 2 and (pc == 3 or pc == 4) or
       player == 3 and (pc == 0 or pc == 4) or
       player == 4 and (pc == 0 or pc == 1)
       )
...
if comeco == 0:
    while True:
        ...
        if comeco == 1:
                ...
                while True:
                    if Pontos_pc == 3:
                        print(emojize( 'Você perdeu, mais sorte na proxima... :cry:',language='alias') )  
                        nova_partida()
                    ...
                    player = int( input( 'Escolha um número referente ao que você quer jogar:\n Pedra = 0 \n Tesoura = 1\n Lagarto = 2\n Papel = 3\n Spock = 4\n Digite aqui:') )
                    print(f'{cores["roxo"]}{'-=-' * 10}{cores["vazio"]}')      
                    sleep(1)
                    pc = randrange(0, 5)
                    ...
                    if player >= 5 or player < 0:
                        ...
                        print(f'Você perdeu esse round!\nSua escolha foi {player} e a do seu oponente foi {pc}')
                        print(f'--Seus Pontos {Pontos_player} x Pontos do Pc {Pontos_pc}--')
                        print(f'{cores["vermelho"]}{'-+-' * 10}{cores["vazio"]}')